...
imageOnProductSchema.pre(/delete/i, async function (next) {
	const toBeDeletedProductImage = await imageOnProductModel.findOne(this._conditions)
	if (!toBeDeletedProductImage) return next()
	await mongoose.model('image').findByIdAndDelete(toBeDeletedProductImage.imageId._id)
})

const imageOnProductModel = mongoose.model('imageOnProduct', imageOnProductSchema)
